class Eonix extends Date {
  ...
  inRange(start, end, options = {}) {
    ...
    const startTime = new Eonix(start).getTime();
    const endTime = new Eonix(end).getTime();

    const startComparison = includeStart ? (d, s) => d >= s : (d, s) => d > s;
    const endComparison = includeEnd ? (d, e) => d <= e : (d, e) => d < e;

    return startComparison(nowTime, startTime) && endComparison(nowTime, endTime);
  }
